const os = require("os");
let { WORKTYPE, PREFIX, BOT_INFO, BASE_URL } = require("../config");
let img_url,
  theam = "text";
...
const { getBuffer, sleep } = require("./main/func");
...
function MediaUrl(text) {
  ...
  if (urls) {
    for (let i = 0; i < urls.length; i++) {
      if (
        ["jpg", "jpeg", "png"].includes(urls[i].split(".").pop().toLowerCase())
      ) {
        array.push({
          image: urls[i],
        });
...
async function broadcast(msg, opt, type = "group") {
  ...
  if (type == "group") {
    const { participants } = await msg.client
      .groupMetadata(msg.jid)
      .catch((e) => []);
    ...
    const groups = Object.entries(response)
      .slice(0)
      .map((entry) => entry[1]);
    ...
    const Jid = await msg.store.chats
      .all()
      .filter((a) => !a.id.endsWith("g.us") && !a.id.endsWith("broadcast"))
      .map((a) => a.id);
    Jid.map(async (j) => {
      await sleep(1500);
