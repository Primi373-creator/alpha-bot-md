...
Alpha({
	...
	if (match.startsWith('dl-id:')) {
		...
		const res = await getJson(`https://upper-romy-inrl-bot.koyeb.app/api/download/apk?url=${match}&apikey=free50_inrl`);
		if (!res.status) return await message.send(`Please enter a new apikey, as the given apikey limit has been exceeded. Visit ${config.BASE_URL}signup for gettig a new apikey. setvar alpha_key: your apikey`);
