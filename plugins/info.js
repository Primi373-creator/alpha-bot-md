...
Alpha(
  ...
  async (message, match) => {
    if (!match)
      return await message.send("*provide a time!*\n*Example: dd/mm/yyyy*");
    ...
    const result = await getJson(
      `${config.BASE_URL}api/info/age?dob=${match}&apikey=${config.INRL_KEY}`,
    );
    if (!result.status)
    ...
    const { age, months, days, hours, minutes, seconds, next } = result.result;
...
Alpha(
  ...
  async (message, match) => {
    ...
    const { result, status } = await getJson(
      `${config.BASE_URL}api/info/country?code=${match}&apikey=${config.INRL_KEY}`,
    );
    if (!status)
      return await message.send(
        `Please enter a new apikey, as the given apikey limit has been exceeded. Visit ${config.BASE_URL}api/signup for gettig a new apikey. setvar inrl_key: your apikey`,
